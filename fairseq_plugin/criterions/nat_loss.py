...
class LabelSmoothedDualImitationCriterionConfig(FairseqDataclass):
    label_smoothing: float = field(
        default=0.0, metadata={"help": "epsilon for label smoothing, 0 means no label smoothing"}
    )
...
class LabelSmoothedDualImitationCriterion(FairseqCriterion):
    ...
    def _compute_loss(self, outputs, targets, masks=None, label_smoothing=0.0, name="loss", factor=1.0):
        ...
        policy_logprob: if there is some policy depends on the likelihood score as rewards.
        """
    ...
    def forward(self, model, sample, reduce=True):
        ...
        for obj in outputs:
            ...
            if self.report_accuracy and outputs[obj].get("report_accuracy", False):
                ...
                logging_output[f"_{obj}@n_correct"] = utils.item(n_correct)
                logging_output[f"_{obj}@total"] = utils.item(mask.sum())
        ...
        if "output_logs" in outputs:
            for k, v in outputs["output_logs"].items():
                logging_output[k] = v
