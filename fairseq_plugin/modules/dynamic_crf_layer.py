...
class DynamicCRF(nn.Module):
    ...
    def forward(self, emissions, targets, masks, beam=None, matrix=None):
        """
        ...
        """
        numerator = self._compute_score(emissions, targets, masks, matrix)
        denominator = self._compute_normalizer(emissions, targets, masks, beam, matrix)
        return numerator - denominator

    def forward_decoder(self, emissions, masks=None, beam=None, matrix=None):
        """
        ...
        """
        return self._viterbi_decode(emissions, masks, beam, matrix)

    def _compute_score(self, emissions, targets, masks=None, matrix=None):
        emission_scores = emissions.gather(2, targets[:, :, None])[:, :, 0]  # B x T
        transition_scores = self.E1(targets[:, :-1])
        if matrix is not None:
            transition_scores *= matrix
        transition_scores = (transition_scores * self.E2(targets[:, 1:])).sum(2)
    ...
    def _compute_normalizer(self, emissions, targets=None, masks=None, beam=None, matrix=None):
        ...
        if matrix is not None:
            beam_transition_score1 *= matrix.unsqueeze(2)
    ...
    def _viterbi_decode(self, emissions, masks=None, beam=None, matrix=None):
        # HACK: we use a beam of tokens to approximate the normalizing factor (which is bad?)
