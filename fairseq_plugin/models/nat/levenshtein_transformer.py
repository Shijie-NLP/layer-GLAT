...
class LevenshteinTransformerModel(NATransformerModel):
    ...
    def forward_decoder(self, decoder_out, encoder_out, eos_penalty=0.0, max_ratio=None, **kwargs):
        ...
        encoder_out = copy.deepcopy(encoder_out)
...
class LevenshteinTransformerDecoder(NATransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if getattr(args, "share_discriminator_maskpredictor", False):
            assert getattr(args, "no_share_discriminator", False), "must set separate discriminator"
            self.layers_msk = self.layers_del

    def extract_features(
        self, prev_output_tokens, encoder_out=None, early_exit=None, return_all_hiddens=False, layers=None, **unused
    ):
        """
    ...
    def forward_mask_ins(self, normalize, encoder_out, prev_output_tokens, **unused):
        ...
        if normalize:
            return F.log_softmax(decoder_out, -1), extra["attns"]
        return decoder_out, extra["attns"]
