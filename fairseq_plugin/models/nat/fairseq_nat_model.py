    ...
    def wrapper(self, normalize=False, encoder_out=None, *args, **kwargs):
        ...
        def _stack(t):
            ...
            if isinstance(t[0], list):
                return [_stack(a) for a in t]

            if isinstance(t[0], dict):
                return [{k: _stack(v) for k, v in d.items()} for d in t]

            raise NotImplementedError(f"Don't know how to stack type {type(t)}")

        ensembled_outs = []
        for i in range(len(action_outs[0])):
            if i == 0 and normalize:
                ensembled_outs += [
                    torch.logsumexp(_stack([a[i] for a in action_outs]), dim=-1) - math.log(len(self.ensemble_models))
                ]
            elif action_outs[0][i] is not None:
                ensembled_outs += [_stack([a[i] for a in action_outs])]
            else:
                ensembled_outs += [None]
