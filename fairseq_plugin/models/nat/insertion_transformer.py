...
from .nonautoregressive_transformer import base_architecture, ensemble_decoder
...
class NegativeDistanceScore(object):
    def __init__(self):
        ...
        self.scores = {
            0.5: self.compute_score_full(50, 0.5),
            1.0: self.compute_score_full(50, 1.0),
            2.0: self.compute_score_full(50, 2.0),
        }
...
def _get_ins_targets(in_tokens, out_tokens, padding_idx, unk_idx, vocab_size, tau=None):
    ...
    flatten = []
    for i, labels in enumerate(insert_labels):  # i is the index of sentence
        for j, label in enumerate(labels[1:-1]):  # j is the index of insert position
            for k, w in enumerate(label):  # k is the number of insert tokens
                flatten.append((w + (j + i * (T - 1)) * V, neg_scorer(k, len(label), tau)))

    insert_index, insert_labels = zip(*flatten)  # HACK 1:-1
...
@register_model_architecture("insertion_transformer", "insertion_transformer_iwslt_de_en")
def insertion_transformer_iwslt_de_en(args):
    ...
    insertion_base_architecture(args)
